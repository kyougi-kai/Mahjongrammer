    ...
    ws.on("message", async (messageData) => {
      ...
      if (message.hasOwnProperty("entryRoom")) {
        ...
        if (message.outRoom == username) {
          ...
          //他の人も強制退出させる処理
          await pool.query("delete from room_member where user_id = ?", [
            userId,
          ]);
          Object.values(playClients[roomId]).forEach((memberWs) => {
            memberWs.send(JSON.stringify({ forcedFinish: true }));
          });
          await pool.query("delete from room_member where room_id = ?", [
            roomId,
          ]);
          await deleteRoom(parentId);
...
app.get("/play/:parentName", async (req, res) => {
  try {
    ...
    if ((await checkValue("rooms", "parent_id", parentId)) == 0)
      res.redirect("/room");
...
app.post("/login", async (req, res) => {
  ...
  try {
    if (
      (await checkValue("users", "username", username)) == 1 &&
      (await checkValue("users", "password", password)) == 1
    ) {
...
async function createRoom(parentId, ratio) {
  await pool.query("insert into rooms (parent_id, ratio) values(?, ?)", [
    parentId,
    ratio,
  ]);
}
...
async function entryRoom(room_id, userId) {
  await pool.query("insert into room_member (room_id, user_id) values(?,?)", [
    room_id,
    userId,
  ]);
}
...
async function checkValue(tableName, fieldName, value) {
  const result = await pool.query(
    `select count(*) from ${tableName} where ${fieldName} = '${value}'`
  );
  return result[0][0]["count(*)"];
}
...
async function addUser(username, password) {
  await pool.query(
    "insert into users (user_id, username, password) values(uuid(),?,?);",
    [username, password]
  );
}
...
async function getRow(tableName, fieldName, filterName, value) {
  const result = await pool.query(
    `select ${fieldName} from ${tableName} where ${filterName} = ?`,
    [value]
  );
...
async function getRows(tableName, filterName, value) {
  const result = await pool.query(
    `select * from ${tableName} where ${filterName} = ?`,
    [value]
  );
  return result[0];
}
