...
export default class gameManager {
  constructor() {
    ...
    const parentName = decodeURIComponent(window.location.pathname).split(
      "/"
    )[2];
    ...
    this._playSocket = new WebSocket(
      `${protocol}://${window.location.host}/play/${parentName}`
    );

    //なく
    this._barkDiv.children[0].addEventListener("click", (event) => {
      const word =
        this._throwHais[this.phaseToPlayerNumber(this._nowPhase)].children[0]
          .textContent;
      let partOfSpeech;
    ...
    this._playSocket.addEventListener("open", () => {
      ...
      this._playSocket.send(
        JSON.stringify({ entryRoom: parentName, username: this._username })
      );
    });
    ...
    this._playSocket.addEventListener("message", (event) => {
      ...
      if (message.hasOwnProperty("forcedFinish")) {
        ...
        this._scoreBord.children[
          this.phaseToPlayerNumber(message.targetNumber)
        ].style.animation = "";
        ...
        this._throwHais[
          this.phaseToPlayerNumber(this._nowPhase)
        ].style.opacity = "0";
        ...
        this._scoreBord.children[
          this.phaseToPlayerNumber(message.bark)
        ].style.animation = "blinking-bark 2s infinite ease";
    ...
    window.onbeforeunload = (event) => {
      if (this._parentFlag)
        this._playSocket.send(
          JSON.stringify({ outRoom: parentName, username: this._username })
        );
    };
  ...
  complete() {
    const targetHais = Array.from(this._hm._divisions).filter(
      (value) => value.style.opacity == "1"
    );
    console.log(targetHais);
    this._playSocket.send(
      JSON.stringify({ complete: document.getElementsByClassName('hai-table')[0].outerHTML, username: this._username })
    );
  }
  ...
  nextPhase() {
    ...
    this._scoreBord.children[
      this.phaseToPlayerNumber(this._nowPhase)
    ].style.animation = "blinking 2s infinite ease";
  ...
  phaseToPlayerNumber(phase) {
    return (2 - this._ownNumber + phase) % 4 == -1
      ? 3
      : (2 - this._ownNumber + phase) % 4;
  }
